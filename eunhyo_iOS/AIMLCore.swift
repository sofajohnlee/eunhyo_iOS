...
// Compatibility aliases retained for earlier menu references.
typealias PDFLibraryView = RichPdfLibraryView
typealias SportsVideosView = SportsView
typealias MagicVideosView = MagicView
...
enum AIMLPatternMatcher {
    ...
    private static func matchFrom(tokens: [String], words: [String], tokenIndex: Int, wordIndex: Int, stars: [String]) -> AIMLMatch? {
        if tokenIndex == tokens.count { return wordIndex == words.count ? AIMLMatch(stars: stars) : nil }
...
struct AIMLSubstitutionTable {
    ...
    func apply(_ value: String) -> String {
        var result = " \(value) "
        for (from, to) in substitutions { result = result.replacingOccurrences(of: from, with: to, options: [.caseInsensitive]) }
    ...
    static func parse(_ text: String) -> AIMLSubstitutionTable {
        let rows = text.split(whereSeparator: \.isNewline).compactMap { raw -> (String, String)? in
            ...
            let fields = splitCSV(line); guard fields.count >= 2, !fields[0].isEmpty else { return nil }
            return (fields[0], fields[1])
        }
        return AIMLSubstitutionTable(substitutions: rows)
    }
    private static func splitCSV(_ line: String) -> [String] {
        var result: [String] = [], current = "", quoted = false; let chars = Array(line); var i = 0
        while i < chars.count {
            let c = chars[i]
            if c == "\"" { if quoted && i + 1 < chars.count && chars[i + 1] == "\"" { current.append("\""); i += 1 } else { quoted.toggle() } }
...
struct AIMLCategory { let pattern: String; let template: String }

final class SwiftAIMLChatEngine {
    private let predicates = AIMLPredicateStore(values: ["name": "친구", "topic": "unknown"])
    private let normalizer = AIMLSubstitutionTable(substitutions: [(" what's ", " what is "), (" i'm ", " i am "), (" can't ", " cannot "), (" 안녕! ", " 안녕 ")])
    ...
    func reply(to rawInput: String) -> String {
        let input = normalizer.apply(rawInput.trimmingCharacters(in: .whitespacesAndNewlines)); if input.isEmpty { return "질문을 입력해 주세요." }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ScrollViewReader { proxy in
                ScrollView { LazyVStack(spacing: 10) { ForEach(messages) { message in HStack { if message.isUser { Spacer(minLength: 50) }; Text(message.text).padding(10).background(message.isUser ? Color.accentColor.opacity(0.16) : Color.secondary.opacity(0.12)).clipShape(RoundedRectangle(cornerRadius: 12)); if !message.isUser { Spacer(minLength: 50) } }.id(message.id) } }.padding() }
                    .onChange(of: messages.count) { _, _ in if let id = messages.last?.id { proxy.scrollTo(id, anchor: .bottom) } }
            }
            Divider(); HStack { TextField("질문을 입력하세요", text: $input); Button("보내기") { send() }.disabled(input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }.padding()
...
struct InteractiveGeometryView: View {
    ...
    var body: some View {
        Form {
            Section("도형 조작") { VStack { Canvas { context, size in let center = CGPoint(x: size.width/2, y: size.height/2); var path = Path(); for i in 0..<3 { let angle = (Double(i) * 120 + rotation - 90) * .pi / 180; let point = CGPoint(x: center.x + CGFloat(cos(angle) * radius), y: center.y + CGFloat(sin(angle) * radius)); if i == 0 { path.move(to: point) } else { path.addLine(to: point) } }; path.closeSubpath(); context.stroke(path, with: .foreground, lineWidth: 4) }.frame(height: 260); Slider(value: $radius, in: 30...110); Slider(value: $rotation, in: 0...360) } }
